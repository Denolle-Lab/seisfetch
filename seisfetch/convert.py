...
class TraceBundle:
    ...
    def to_dict(self) -> dict[str, np.ndarray]:
        """``{nslc_id: ndarray}`` — segments with same ID concatenated."""
        groups: dict[str, list[np.ndarray]] = {}
        for t in self.traces:
            groups.setdefault(t.id, []).append(t.data)
...
def parse_mseed(raw: bytes) -> TraceBundle:
    ...
    for msr in MS3Record.from_buffer(raw, unpack_data=True):
        ...
        traces.append(TraceArray(
            network=net, station=sta, location=loc, channel=cha,
            starttime_ns=msr.starttime, sampling_rate=msr.samprate,
            data=arr,
        ))
...
def bundle_to_obspy(bundle: TraceBundle):
    ...
    for t in bundle.traces:
        tr = Trace(data=t.data, header={
            "network": t.network, "station": t.station,
...
def bundle_to_inventory(bundle: TraceBundle, provider: str = "EARTHSCOPE"):
    ...
    **Requires ObsPy.**  Uses ObsPy's FDSN client for metadata only.
    ...
    nslcs = {(t.network, t.station, t.location, t.channel)
             for t in bundle.traces}
    inv = None
    for net, sta, loc, cha in nslcs:
        try:
            ...
            logger.warning("Could not fetch inventory for %s.%s.%s.%s",
                           net, sta, loc, cha, exc_info=True)
    return inv
...
def bundle_to_xarray(bundle: TraceBundle, merge_segments=True):
    ...
    for nslc, segs in grouped.items():
        ...
        data_vars[var_name] = xr.DataArray(
            data=all_data, dims=["time"], coords={"time": times},
            attrs={"network": segs[0].network, "station": segs[0].station,
                   "location": segs[0].location, "channel": segs[0].channel,
                   "sampling_rate": sr, "units": "counts"},
        )
...
def to_zarr(ds_or_bundle, store: str, mode="w", **zarr_kwargs):
    ...
    **Requires xarray + zarr.**
    ...
    if isinstance(ds_or_bundle, TraceBundle):
        ...
        raise TypeError(f"Expected TraceBundle or xarray.Dataset, got {type(ds_or_bundle)}")
